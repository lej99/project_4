$(document).ready(function(){
    ...
    $.ajax({
        ...
        for (var i = 0; i < 18; i++) {
            ...
            li[i].innerHTML += '<a href="./sub.html" class="thumbnail"><img src="' + msg.documents[i].thumbnail + '"></a>'
            + '<h4 class="num">' + (i+1) + '</h4>'
            + '<div><a href="./sub.html"><h4 class="title">' + msg.documents[i].title + '</h4></a>'
            + '<a href="./sub.html"><p class="authors">' + msg.documents[i].authors[0] + '</p></a>'
    ...
    $.ajax({
        ...
        for (var i = 0; i < 8; i++) {
            ...
            li[i].innerHTML += '<a href="./sub.html" class="thumbnail"><img src="' + msg.documents[i].thumbnail + '"></a>'
            + '<a href="./sub.html"><h4 class="title">' + msg.documents[i].title + '</h4></a>'
            + '<a href="./sub.html" class="authors"><p>' + msg.documents[i].authors[0] + '</p></a>'
            + '<p class="stars"><svg fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="구매자 평균 별점"><path fill-rule="evenodd" clip-rule="evenodd" d="M5.5 0l1.812 3.303L11 4.025 8.446 6.759l.459 3.741L5.5 8.903 2.095 10.5l.459-3.74L0 4.002l3.688-.7L5.5 0z" fill="currentColor"></path></svg>'
            + '<span class="reviews">' + (Math.random() * (5 - 4) + 4).toFixed(1) + '</span>'
